...
CHALLENGES = {
    "level_0": [
        ("0_reading", "Книга", "Читать 1 страницу каждый день 7 дней.", "media"),
        ("0_walking", "Поймать мысль", "Записывать 1 мысль в дневник.", "media"),
        ("0_warmup", "Зарядка", "Зарядка 1 минуту ежедневно.", "media"),
        ("0_pushups", "Отжимания", "1 отжимание каждый день.", "media"),
        ("0_learnings", "Изучение", "Учить 1 новое слово.", "media"),
        ("0_squats", "Благодарность", "Каждый день благодарить за что-то одно.", "media"),
        ("0_plank", "Подкаст", "Слушать 1 минуту подкаста/книги.", "media"),
        ("0_jumps", "Уединение", "Сидеть в тишине 1 минуту.", "media"),
        ("0_splits", "Шпагат", "Тянуться 1 минуту.", "media"),
        ("0_lay_bad", "Кровать", "Заправлять кровать после пробуждения.", "media"),
        ("0_not_touch", "Телефон", "Не открывать соцсети 1 минуту.", "media"),
        ("0_money", "Фин. Грамотность", "Откладывать по 1 монете/гривне/доллару.", "media"),
    ],
    "level_1": [
...
async def accept_challenge(callback: types.CallbackQuery):
    ...
    async with pool.acquire() as conn:
        ...
        # сколько раз завершён → это уровень награды
        row = await conn.fetchrow("""
            SELECT repeat_count FROM completed_challenges
            WHERE user_id=$1 AND challenge_id=$2
        """, user_id, cid)
        ...
        repeat = row["repeat_count"] if row else 0
        difficulty = min(repeat + 1, 3)      # сложность = 1 / 2 / 3
        days = 7 if repeat == 0 else 10 if repeat == 1 else 14
